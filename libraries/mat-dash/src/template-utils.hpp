...
template <typename R, typename... Args>
struct AddStdcall<R(Args...)> {
    using type = typename R(__stdcall*)(Args...);
};
...
struct Orig<func, Stdcall, void> {
    inline static typename AddStdcall<decltype(func)>::type orig;
};
...
template<class R, class... Args>
struct stdcall<R(*)(Args...)> : public stdcall<R(Args...)> {};

template<class R, class... Args>
struct stdcall<R(Args...)> {
    using F = R(Args...);
    template <F func>
    static R __stdcall wrap(Args... args) { return func(args...); }
};
